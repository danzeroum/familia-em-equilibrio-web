...
export function useSocialEventExpenses() {
  ...
  async function upsert(item: Partial<SocialEventExpense>) {
    ...
    if (payload.id) {
      ...
      const { error } = await db.from('social_event_expenses').update(updateData).eq('id', payload.id)
      if (error) console.error('[useSocialEventExpenses] update error:', error.message)
    } else {
      const { data: { user } } = await supabase.auth.getUser()
      const { error } = await db.from('social_event_expenses').insert({
        ...payload,
        family_id: fid,
  ...
  async function remove(id: string) {
    const { error } = await db.from('social_event_expenses').delete().eq('id', id)
