...
export function ChatbotModal() {
  ...
  return (
    ...
    >
      {/* Header — sempre visível, clicável para minimizar */}
      <div
        ...
        <div className="flex-1 min-w-0">
          ...
          {!minimized && (
            <p className="text-xs opacity-75 truncate">
              Cole listas ou mensagens do WhatsApp
            </p>
          )}
        </div>
      ...
      {!minimized && (
        <>
          ...
          >
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center py-8 gap-3 text-center px-4">
                ...
                <p className="text-xs font-medium text-text">Cole qualquer texto aqui</p>
                    ...
                    dangerouslySetInnerHTML={{
                      __html:
                        msg.content
                          ...
                          .replace(/\n/g, '<br/>') +
                        (msg.streaming
                          ? '<span style="display:inline-block;width:8px;height:12px;background:currentColor;margin-left:2px;border-radius:1px;animation:blink 1s step-end infinite;vertical-align:text-bottom;opacity:0.7"></span>'
                          : ''),
                    }}
                  />
                </div>
              ...
              <div className="bg-surface border border-border rounded-xl overflow-hidden">
                ...
                <div className="px-3 py-2 flex gap-2 border-t border-border bg-surface-offset">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 bg-primary hover:bg-primary-hover text-white text-xs font-medium py-2 rounded-xl transition-colors disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-2 text-xs text-text-muted hover:text-text border border-border rounded-xl transition-colors"
                  >
                    Cancelar
                  </button>
                </div>
            ...
            {loading && !preview && (
              <div className="flex justify-start">
                <div className="bg-surface border border-border rounded-2xl rounded-bl-sm px-3 py-2 flex items-center gap-2 text-xs text-text-muted">
          ...
          {/* Input */}
          <div className="p-3 border-t border-border bg-bg shrink-0">
            <div className="flex gap-2 items-end bg-surface border border-border rounded-xl px-3 py-2 focus-within:border-primary transition-colors">
              <textarea
                ...
                className="flex-1 resize-none bg-transparent text-xs text-text placeholder:text-text-faint outline-none max-h-24 min-h-[2rem]"
                rows={2}
              ...
              <button
                ...
                className="flex-shrink-0 w-8 h-8 rounded-lg bg-primary hover:bg-primary-hover disabled:opacity-40 disabled:cursor-not-allowed text-white flex items-center justify-center transition-colors"
              >
                {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Send className="w-3 h-3" />}
              </button>
            </div>
            <p className="text-[10px] text-text-faint mt-1.5 text-center">Ctrl+Enter para enviar</p>
          </div>
        </>
      )}
  ...
  return (
    <div className="px-3 py-2">
      <div className="flex items-start gap-2">
        <span className={`inline-flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded-full font-medium shrink-0 mt-0.5 ${config.bgColor} ${config.textColor}`}>
          {config.emoji}
        </span>
        <div className="flex-1 min-w-0">
          <input
            ...
            className="w-full text-xs text-text bg-transparent border-b border-transparent hover:border-border focus:border-primary outline-none transition-colors pb-0.5"
          />
          {(item.quantity || item.date || item.recurrence) && (
            <div className="flex flex-wrap gap-x-2 mt-0.5">
              {item.quantity && <span className="text-[10px] text-text-muted">Qtd: {item.quantity}</span>}
              {item.date && <span className="text-[10px] text-text-muted">📅 {item.date}</span>}
              {item.recurrence && <span className="text-[10px] text-text-muted">🔄 {item.recurrence}</span>}
            </div>
          )}
        </div>
        <span className={`text-[10px] font-mono shrink-0 ${item.confidence >= 0.8 ? 'text-success' : item.confidence >= 0.5 ? 'text-warning' : 'text-error'}`}>
      ...
      {expanded && (
        <div className="mt-2 grid grid-cols-2 gap-1.5 pl-1">
          <div>
            <label className="text-[10px] text-text-muted block mb-0.5">Tipo</label>
            <select
              value={item.type}
              onChange={(e) => onUpdate(index, { type: e.target.value as ParsedItemType })}
              className="w-full text-[10px] bg-surface-offset border border-border rounded-lg px-2 py-1 text-text outline-none focus:border-primary"
            >
              {TYPE_OPTIONS.map((t) => (
          ...
          <div>
            <label className="text-[10px] text-text-muted block mb-0.5">Quantidade</label>
            <input value={item.quantity ?? ''} onChange={(e) => onUpdate(index, { quantity: e.target.value || null })} placeholder="ex: 2" className="w-full text-[10px] bg-surface-offset border border-border rounded-lg px-2 py-1 text-text outline-none focus:border-primary" />
          </div>
          <div className="col-span-2">
            <label className="text-[10px] text-text-muted block mb-0.5">Observações</label>
            <input value={item.notes ?? ''} onChange={(e) => onUpdate(index, { notes: e.target.value || null })} placeholder="Informações adicionais..." className="w-full text-[10px] bg-surface-offset border border-border rounded-lg px-2 py-1 text-text outline-none focus:border-primary" />
          </div>
        </div>
      )}
    </div>
