...
interface Props { open: boolean; onClose: () => void; task: Task | null; onSave: (t: any) => Promise<void>; members: Profile[] }

export function TaskSheet({ open, onClose, task, onSave, members }: Props) {
  const [form, setForm] = useState<Partial<Task>>({})
  useEffect(() => { setForm(task ?? { priority: 'medium', status: 'pending', recurrence: 'none' }) }, [task])
  ...
  return (
    <SlideOver title={task ? 'Editar tarefa' : 'Nova tarefa'} onClose={onClose}>
      ...
      <div>
        ...
        <select className="input-base" value={form.assigned_to ?? ''} onChange={e => set('assigned_to', e.target.value)}>
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" checked={!!form.requires_adult_validation} onChange={e => set('requires_adult_validation', e.target.checked)} id="adult-val" />
