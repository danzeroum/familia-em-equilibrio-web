...
export default function EducacaoPage() {
  ...
  const handleBulkAddSupplies = async (names: string[]) => {
    for (const name of names) {
      await supplies.upsert({
        ...
        status: 'needed',
        category: 'material',
        profile_id: filterMember === 'all' ? null : filterMember,
        quantity_have: 0,
        quantity_need: 1,
      } as Omit<SchoolSupply, 'id' | 'family_id' | 'created_at' | 'updated_at'>)
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ══ BLOCO A — COMPRAS ESCOLARES ══ */}
      {tab === 'compras' && (
        <div className="space-y-5">
          <QuickAddList
            onAdd={handleBulkAddSupplies}
            placeholder="Digite ou cole a lista de material escolar..."
          />

          {supplies.isLoading ? (
            <div className="p-8 text-center text-gray-400">Carregando...</div>
          ) : allSupplies.length === 0 ? (
            <EmptyState emoji="🛒" title="Lista vazia" description="Adicione itens escolares no campo acima — material básico, uniformes, livros." />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {boughtCompras.map(i => (
                      <div key={i.id} className="flex items-center justify-between bg-gray-50 border p-2 rounded text-sm">
                        ...
                        <button onClick={() => handleToggleBuy(i)} className="text-teal-600 font-medium text-xs bg-teal-50 px-2 py-1 rounded hover:bg-teal-100">Repor</button>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {tab === 'material' && (
        <div className="rounded-xl border bg-white overflow-hidden">
          {supplies.isLoading ? (
            ...
            <EmptyState emoji="📦" title="Nenhum material cadastrado" description="Cadastre a lista inicial, reposições e uniformes para etiquetar." />
          ) : (
      ...
      {tab === 'licoes' && (
        <div className="rounded-xl border bg-white overflow-hidden">
          {homework.isLoading ? (
            ...
            <EmptyState emoji="✏️" title="Nenhuma lição cadastrada" description="Tarefas diárias, projetos e prazos das crianças." />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wide">
                      ...
                      <tr key={i.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2 justify-end">
                            ...
                            <button title="Criar agendamento" onClick={() => schedule(`📝 ${i.subject}: ${i.title}`, i.due_date)} className="text-xs text-blue-400 hover:text-blue-600">📅</button>
                            <button className="text-xs text-teal-600 hover:underline"
      ...
      {tab === 'comunicacao' && (
        <div className="rounded-xl border bg-white overflow-hidden">
          {communications.isLoading ? (
            ...
            <EmptyState emoji="💬" title="Nenhuma comunicação registrada" description="Grupos WhatsApp, e-mails de professores, reuniões — tudo num lugar só." />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wide">
                      ...
                      <tr key={i.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2 justify-end">
                            ...
                            <button title="Criar agendamento" onClick={() => schedule(`📩 Comunicado: ${i.title}`, i.due_date)} className="text-xs text-blue-400 hover:text-blue-600">📅</button>
                            <button className="text-xs text-teal-600 hover:underline"
      ...
      {/* Sheets */}
      <SchoolCommunicationSheet open={commOpen} onClose={() => setCommOpen(false)} item={selectedComm} onSave={communications.upsert} members={members} />
      <SchoolHomeworkSheet open={hwOpen} onClose={() => setHwOpen(false)} item={selectedHw} onSave={homework.upsert} members={members} />
      <SchoolSupplySheet open={supplyOpen} onClose={() => setSupplyOpen(false)} item={selectedSupply} defaultCategory={supplyDefaultCategory} onSave={supplies.upsert} members={members} />
      <AgendamentoSheet
        open={schedOpen}
